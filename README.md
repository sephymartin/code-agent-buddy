...
这是一个面向 AI 编码助手的规范与技能仓库，当前主要用于沉淀 Java 后端开发相关的写码约束、数据库建模规则，以及按需加载的 MyBatis-Plus 持久层实践。
...
- `java-code-writing`
  - 作为 Java 后端开发的单一入口 skill
  - 主体覆盖：Lombok 使用、构造注入、事务边界、工具类写法、同步服务边界等
  - 按需加载 references：MyBatis-Plus 的 `*DO` / `*Dao` / 分页 / 批量 / 局部更新规则，以及数据库字段、索引、长文本拆表和 DO 映射约定
- `database-design`
  - 面向偏设计文档场景的数据库建模输出
...
- 让代理在确实涉及持久层时，再按 MyBatis-Plus 约定实现 Dao、DO、分页和批量操作
- 让代理输出贴合现有规范的数据库设计方案
...
- 使用 `java-code-writing` 处理 `*DO`、`*Dao`、分页、批量、局部更新等持久层任务，代理再按需加载对应 references
...
`code-agent-buddy` 当前可以理解为一个“面向 AI 编码代理的 Java 后端规范知识库”。它已经沉淀了 Java 编码、按需加载的 MyBatis-Plus 持久层规则、以及后端数据库建模能力，适合作为团队统一代理行为和生成质量的基础仓库。

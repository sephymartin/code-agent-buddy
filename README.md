...
- 让代理为 `sephy-app` 输出贴合现有规范的数据库设计方案
...
- 使用 `database-modeling` 输出数据库设计章节

其中 `database-modeling/agents/openai.yaml` 已声明该技能可被隐式调用，并给出了默认提示语。
...
`code-agent-buddy` 当前可以理解为一个“面向 AI 编码代理的 Java 后端规范知识库”。它已经沉淀了 Java 编码、MyBatis-Plus 持久层、以及 `sephy-app` 数据库建模三类核心能力，适合作为团队统一代理行为和生成质量的基础仓库。
